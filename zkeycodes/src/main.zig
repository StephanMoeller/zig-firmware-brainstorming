...
fn writeHeader(writer: anytype, out_type: OutputType, lang: []const u8, version: []const u8) !void {
    ...
    try writer.print("const localcore = @import(\"../src/core.zig\");\n", .{});
    try writer.print("const core = @import(\"zigmkay\").core;\n", .{});

    if (out_type == .alias) {
        // Pull modifier helpers into scope so alias expressions compile.
        try writer.print("const L_SFT = localcore.L_SFT;\n", .{});
        try writer.print("const L_ALT = localcore.L_ALT;\n", .{});
        try writer.print("const R_ALT = localcore.R_ALT;\n", .{});
        try writer.print("const R_CTL = localcore.R_CTL;\n", .{});
        try writer.print("const DEAD = localcore.DEAD;\n", .{});
