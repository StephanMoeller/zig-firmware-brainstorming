...
test "Struct size: KeyDef" {
    try std.testing.expectEqual(26, @sizeOf(core.KeyDef));
}
test "Struct size: TapDef" {
    try std.testing.expectEqual(16, @sizeOf(core.TapDef));
}
...
test "Struct size: TapHold" {
    try std.testing.expectEqual(24, @sizeOf(core.TapHoldDef));
}
test "Struct size: AutoFireDef" {
    try std.testing.expectEqual(20, @sizeOf(core.AutoFireDef));
}
...
test "Keymap of 36 keys with 5 layers - sanity checking" {
    // roughly 4kb
    try std.testing.expectEqual(4680, @sizeOf([5][36]core.KeyDef));
}
